/**
 * Chat Widget Color Theme System
 * Derives a complete color palette from a single base color
 * passed via the data-colour attribute on the embed script tag.
 */
 ...
 * Ensures readable text on all backgrounds regardless of how dark/light
 * the base color is.
 */
export function generateColorPalette(baseColor: string): ColorPalette {
  ...
  // For very dark base colors, push the subtle/bot-message shade well into
  // the light range so dark text remains readable on top.
  const subtleLightness = Math.max(Math.min(l + 35, 96), 92);
  const subtleSaturation = l < 30 ? Math.max(s - 40, 5) : Math.max(s - 30, 10);
  ...
  return {
    ...
    surface: '#ffffff',
    surfaceHover: hslToHex(h, Math.max(subtleSaturation - 5, 0), 98),

    text: '#023047',
    textSecondary: '#396d7c',

    border: hslToHex(h, Math.max(subtleSaturation, 10), 88),
    shadow: `rgba(0, 0, 0, 0.08)`,

    userMessage: baseColor,
    botMessage: hslToHex(h, subtleSaturation, subtleLightness),
    userMessageText: isLight ? '#023047' : '#ffffff',
    botMessageText: '#023047',
  };
}
...
export function getThemeColor(searchParams: URLSearchParams): string {
  return searchParams.get('color') || '#219EBC';
}
